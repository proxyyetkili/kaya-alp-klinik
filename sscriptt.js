document.addEventListener('DOMContentLoaded', () => {
    ...
    const rosterDoctors = Array.isArray(window.KAYA_ALP_DOCTORS) ? window.KAYA_ALP_DOCTORS : [];
    const rosterCardMarkup = doctor => `<a class="source-team-card" href="doktor.html?hekim=${doctor.id}" aria-label="${doctor.name} profilini inceleyin"><img src="${doctor.image}" alt="${doctor.alt}" loading="lazy"><span><strong>${doctor.name}</strong><small>${doctor.area}</small><em>Profili inceleyin <i class="fa-solid fa-arrow-right"></i></em></span></a>`;
    ...
    if (doctorsPageGrid) {
        ...
        if (rosterDoctors.length) {
            if (doctorsPageIntro?.classList.contains('page-intro')) doctorsPageIntro.innerHTML = '<p class="eyebrow">HEKİMLERİMİZ</p><h2>Uzmanlık, güven ve<br><span class="accent">ortak bir vizyon.</span></h2>';
            doctorsPageGrid.className = 'source-team-grid doctors-page-grid doctors-page-grid--roster';
            doctorsPageGrid.removeAttribute('data-auto-carousel');
            doctorsPageGrid.innerHTML = rosterDoctors.map(rosterCardMarkup).join('');
        }
    }
    const homeRosterGrid = document.querySelector('.source-team-grid[data-auto-carousel]');
    if (homeRosterGrid && rosterDoctors.length) homeRosterGrid.innerHTML = rosterDoctors.map(rosterCardMarkup).join('');
    ...
    if (doctorProfileRoot) {
        const doctors = {
            ...Object.fromEntries(rosterDoctors.map(doctor => [doctor.id, {
                ...doctor,
                lead: `${doctor.area} alanında, kişisel ihtiyaçları ve klinik bulguları birlikte değerlendirerek anlaşılır bir tedavi planı sunmayı hedefler.`,
                bio: 'İlk görüşmeden itibaren beklentileri dinleyen, uygulanabilir seçenekleri açıkça paylaşan ve tedavi sürecinin her adımında hasta konforunu gözeten bir yaklaşımı benimser.',
                focus: doctor.area.split(' · ').filter(Boolean),
                approach: 'Muayene bulguları ve ihtiyaçlarınıza göre, tüm seçeneklerin açıkça konuşulduğu kişiye özel bir plan oluşturulur.'
            }])),
        ...
        const otherDoctors = Object.entries(doctors).filter(([key]) => key !== doctorKey).slice(0, 3).map(([key, item]) => `<a class="doctor-profile-related__card" href="doktor.html?hekim=${key}"><span class="doctor-profile-related__media"><img src="${item.image}" alt="${item.alt}" loading="lazy"></span><span class="doctor-profile-related__content"><span class="doctor-profile-related__area">${item.area}</span><strong>${item.name}</strong><span class="doctor-profile-related__link">Profili inceleyin <i class="fa-solid fa-arrow-right"></i></span></span></a>`).join('');
